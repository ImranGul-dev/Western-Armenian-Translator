...
function clickTranslatorAction(action: string) {
  const button = document.querySelector<HTMLButtonElement>(`[data-translator-action="${action}"]`);
  button?.click();
}

function downloadCurrentTranslation() {
  const result = document.querySelector<HTMLElement>("[data-translator-result='true']");
  const text = result?.dataset.hasResult === "true" ? result.textContent?.trim() : "";
  if (!text) return;
...
function useCurrentTranslationReady() {
  ...
  useEffect(() => {
    const update = () => {
      const result = document.querySelector<HTMLElement>("[data-translator-result='true']");
      setReady(result?.dataset.hasResult === "true");
    };
      ...
      <section className={styles.workspace} aria-label="Western Armenian translation workspace">
        <div className={styles.translatorColumn}>
          ...
          <div className={styles.quickActions} aria-label="Quick actions">
            <button type="button" className={styles.quickAction} onClick={() => clickTranslatorAction("voice-input")}>
              <span className={styles.quickIcon} aria-hidden="true">🎤</span>
              <span><strong>Voice Input</strong><small>Speak to translate</small></span>
            </button>

            <button type="button" className={styles.quickAction} onClick={() => clickTranslatorAction("paste-text")}>
              <span className={styles.quickIcon} aria-hidden="true">▣</span>
              <span><strong>Paste Text</strong><small>Paste from clipboard</small></span>
            </button>

            <button type="button" className={styles.quickAction} onClick={() => clickTranslatorAction("clear-all")}>
              <span className={styles.quickIcon} aria-hidden="true">×</span>
        ...
        <div className={styles.learningColumn}>
          <section className={styles.learningCard}>
            ...
            <div className={styles.learningTools}>
              ...
              <button type="button" className={styles.learningTool} onClick={() => clickTranslatorAction("text-to-speech")} disabled={!translationReady}>
