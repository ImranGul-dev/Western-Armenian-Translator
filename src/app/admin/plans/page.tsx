  ...
  return (
    <ProtectedRoute roles={["admin"]}>
      <DashboardShell
        ...
        description="Control public access, account allowances, app plan settings and feature limits. Paid checkout pricing is managed in Tun WooCommerce."
      >
        ...
        <section className="dashboard-card">
          <div className="card-heading">
            <div>
              <h2>WooCommerce billing</h2>
              <p>Paid checkout, tax, payment methods and subscription billing are managed on Tun. This page controls app access and plan limits only.</p>
            </div>
          </div>
        </section>
        ...
        <section className="plan-admin-grid">
          {plans.map(plan => (
            <article className="dashboard-card plan-admin-card" key={plan.id}>
              <div className="card-heading">
                <div>
                  <span className="plan-label">{plan.slug}</span>
                  <h2>{plan.name}</h2>
                </div>
                {plan.slug !== "free" && <span className="status-chip approved">WooCommerce billing</span>}
              </div>
              ...
              <div className="settings-grid single-column">
                ...
                {plan.slug !== "free" && (
                  <label className="checkbox-label"><input type="checkbox" checked={plan.active} onChange={event => updatePlan(plan.slug, { active: event.target.checked })} />Plan enabled in app</label>
                )}
              </div>

              {plan.slug !== "free" && <p className="form-help">Price, tax, payment methods and subscription settings are managed in Tun WooCommerce.</p>}
