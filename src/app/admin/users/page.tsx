...
function billingSourceLabel(value: EffectivePlan["source"]): string {
...
function UserPlanControl({ row, onSaved, setMessage }: { row: UserRow; onSaved: () => Promise<void>; setMessage: (value: string) => void }) {
  ...
  const paidBillingActive = Boolean(
    row.subscription
    && row.subscription.status === "active"
    && !row.subscription.access_suspended
  );

  async function save() {
    const removing = draft.selection === "billing";
    const warning = draft.selection === "free" && paidBillingActive
      ? "This user still has an active paid subscription. A manual Free override changes application access only and does not cancel billing. Continue?"
      : removing
        ? "Remove the manual override and return this user to billing/default plan resolution?"
  ...
  return (
    <div className="manual-plan-control">
      <select aria-label={`Manual plan for ${row.email || row.id}`} value={draft.selection} onChange={event => setDraft(current => ({ ...current, selection: event.target.value as OverrideDraft["selection"] }))}>
        <option value="billing">Use billing/default</option>
        <option value="free">Free</option>
        <option value="premium">Person</option>
      ...
      {draft.selection === "free" && paidBillingActive && (
        <small className="warning-text">This does not cancel the active paid subscription.</small>
      )}

      <button className="primary-button" disabled={busy} onClick={() => void save()}>
        {busy ? "Saving…" : "Save plan control"}
      </button>
    </div>
  );
}
...
export default function AdminUsersPage() {
  ...
  const visible = useMemo(() => rows.filter(row => {
    ...
    const paid = row.effective_plan.source === "woocommerce";
  ...
  return (
    <ProtectedRoute roles={["admin"]}>
      <DashboardShell
        admin
        title="Users and plan access"
        description="Review WooCommerce billing state, effective application plans, manual grants, activity and roles without creating fake billing records."
      >
        ...
        <section className="dashboard-card">
          <div className="card-heading">
            <div>
              ...
              <p>{visible.length} matching users.</p>
            </div>

            <div className="table-actions">
              <input placeholder="Search name, email, country or user ID" value={search} onChange={event => setSearch(event.target.value)} />
              <select value={subscriberFilter} onChange={event => setSubscriberFilter(event.target.value)}>
                <option value="all">All users</option>
                <option value="paid">WooCommerce plan</option>
          ...
          <div className="admin-table-wrap">
            <table className="admin-table users-plan-table">
              <thead>
                <tr>
                  <th>User</th>
                  <th>Country</th>
              ...
              <tbody>
                {visible.map(row => (
                  <tr key={row.id}>
                    ...
                    <td>
                      <small>Joined: {new Date(row.created_at).toLocaleDateString()}</small>
                      <small>Last active: {row.last_active_at ? new Date(row.last_active_at).toLocaleString() : "Never"}</small>
                    </td>

                    <td>
                      <strong>{row.subscription?.plan_slug === "premium" ? "Person" : row.subscription?.plan_slug === "business" ? "Elite" : "None"}</strong>
                      <span className="status-pill">{row.subscription?.access_suspended ? "paused" : row.subscription?.status || "none"}</span>
                      {row.subscription?.billing_provider === "woocommerce" && row.subscription?.woocommerce_subscription_id && (
                        <small>WooCommerce #{row.subscription.woocommerce_subscription_id}</small>
                      )}
                    </td>

                    <td>
                      <strong>{row.effective_plan.name}</strong>
                      <small>Source: {billingSourceLabel(row.effective_plan.source)}</small>
                      {row.override && <small>Override record: {row.override.plan_slug}{row.override.expires_at ? ` · ${new Date(row.override.expires_at).getTime() <= Date.now() ? "expired" : "expires"} ${new Date(row.override.expires_at).toLocaleString()}` : " · no expiration"}</small>}
                      <small>Widget: {row.effective_plan.widget_enabled ? `Eligible (${row.effective_plan.widget_site_limit} sites)` : "Not eligible"}</small>
                    </td>

                    <td>
                      {row.role === "admin"
                        ? <small>Administrator limits take priority over billing and manual overrides.</small>
                        : <UserPlanControl row={row} onSaved={load} setMessage={setMessage} />}
                    </td>

                    <td>
                      <select value={row.role} onChange={event => void updateRole(row.id, event.target.value as ProfileRole)}>
                        <option value="user">User</option>
                        <option value="language_editor">Language editor</option>
