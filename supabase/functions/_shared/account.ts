...
import type { AccountContext, PlanConfig } from "./types.ts";
...
export function planFromEffective(value: unknown): PlanConfig {
  ...
  const slug = plan.slug === "premium" || plan.slug === "business" || plan.slug === "admin"
    ? plan.slug
    : "free";
  const source = plan.source === "manual" || plan.source === "stripe" || plan.source === "admin"
    ? plan.source
    : "default";
  return {
    ...
    rateLimitPerMinute: numberValue(plan.rate_limit_per_minute, FALLBACK_FREE.rateLimitPerMinute),
...
async function authenticatedUser(admin: SupabaseClient, request: Request): Promise<User | null> {
...
async function anonymousPlan(admin: SupabaseClient): Promise<PlanConfig> {
  const [{ data: freeRow }, { data: settingRow }] = await Promise.all([
    admin.from("plans").select("id,slug,name,monthly_character_limit,max_characters_per_request,history_limit,rate_limit_per_minute,widget_enabled,widget_site_limit,widget_monthly_character_limit,widget_branding_removable").eq("slug", "free").maybeSingle(),
    admin.from("platform_settings").select("value").eq("key", "anonymous_usage").maybeSingle()
  ]);
  const freePlan = planFromEffective(freeRow);
  return anonymousFromSettings(freePlan, settingRow?.value);
}
...
export async function resolveEffectivePlan(admin: SupabaseClient, userId: string): Promise<PlanConfig> {
  ...
  if (error) return FALLBACK_FREE;
  return planFromEffective(data);
}

export async function resolveAccount(admin: SupabaseClient, request: Request, anonymousHash: string): Promise<AccountContext> {
  ...
  return {
    ...
    plan: await resolveEffectivePlan(admin, user.id)
  };
}
